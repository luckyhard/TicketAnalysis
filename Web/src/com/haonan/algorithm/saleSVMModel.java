...
public class saleSVMModel {
	public static void main(String[] args) {
		...
		options.put("dbtable", "train_saleok_log");
		options.put("user", "root");
		options.put("password", "123456");
		Dataset<Row> data = session.read().format("jdbc")
				.options(options).load();
		data.registerTempTable("train_saleok_log");
		Dataset<Row> rows = data.sqlContext().sql("select * from train_saleok_log");
		...
		JavaRDD<LabeledPoint> points = rowRDD.map(new Function<Row, LabeledPoint>() {
			...
			public LabeledPoint call(Row row) throws Exception {
				...
				double[] features = {round/num,time};
				return new LabeledPoint(label,Vectors.dense(features));
			}
		});
		
		SVMModel model = SVMWithSGD.train(points.rdd(), 200, 0.00001,1);
		double[] weight = model.weights().toArray();
		System.out.println(model);
		System.out.println(weight.length);
		System.out.println(df.format(weight[0])+" "+df.format(weight[1]));
		model.save(session.sparkContext(), "saleSVM");
